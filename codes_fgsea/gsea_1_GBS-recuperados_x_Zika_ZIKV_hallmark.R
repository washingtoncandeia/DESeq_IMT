...
# Obs.: Este código contém todas as 5 análises:
# H, C2, C5, C7
##---------------------------
library(tidyverse)
library(fgsea)
...
ggplot(fgseaRes, aes(reorder(pathway, NES), NES)) +
  ...
  labs(x="Vias", y="Escore Enriquecido Normalizado (NES)",
       title="Principais Vias de Sujeitos GBS (por ZIKV) Vs ZIKV (recuperados)") + 
  theme_minimal()
...
# Análise 2: C2-KEGG pathways
fgsea(pathways=gmtPathways("symbols/c2.cp.kegg.v7.0.symbols.gmt"), ranks, nperm=1000) %>% 
  as_tibble() %>% 
  arrange(padj)

# Análise 3: C5-GO Annotations
fgsea(pathways=gmtPathways("symbols/c5.all.v7.0.symbols.gmt"), ranks, nperm=1000) %>% 
  as_tibble() %>% 
  arrange(padj)

# Análise 4:C7-Immunological Signatures (ImmuneSigDB)
fgsea(pathways=gmtPathways("symbols/c7.all.v7.0.symbols.gmt"), ranks, nperm=1000) %>% 
  as_tibble() %>% 
  arrange(padj)
